...
function authHeader() {
  ...
  if (!u || !p) throw new Error('Faltan METEOMATICS_USERNAME/METEOMATICS_PASSWORD en .env');
...
export async function getMeteomatics(opts: {
  ...
  if (!params?.length) throw new Error('params requerido');

  let timePart: string;
  if (startISO && endISO) timePart = `${startISO}--${endISO}:${timestep}`;
  else if (validISO)      timePart = validISO;
  else throw new Error('Provee validISO o (startISO y endISO)');
